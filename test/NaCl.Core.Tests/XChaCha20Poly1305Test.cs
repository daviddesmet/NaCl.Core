...
[Category("CI")]
public class XChaCha20Poly1305Test(ITestOutputHelper output)
{
    private const string ExceptionMessageNonceLength = "*The nonce length in bytes must be 24.";
    ...
    public void ModifiedCiphertextFails()
    {
        ...
        for (var b = 0; b < ciphertext.Length; b++)
        {
            for (var bit = 0; bit < 8; bit++)
            {
                ...
                var act = () => aead.Decrypt(nonce, modified, tag, decrypted, aad);
                act.ShouldThrow<CryptographicException>(SnufflePoly1305.AEAD_EXCEPTION_INVALID_TAG);
            }
        }
        ...
        for (var b = 0; b < aad.Length; b++)
        {
            for (var bit = 0; bit < 8; bit++)
            {
                ...
                var decrypted = new byte[ciphertext.Length];
                var act = () => aead.Decrypt(nonce, ciphertext, tag, decrypted, modified);
                act.ShouldThrow<Exception>();
            }
        }
    }
    ...
    */
    ...
    public void ModifiedAssociatedDataFails()
    {
        ...
        for (var msgSize = 0; msgSize < 75; msgSize++)
        {
            ...
            var badAadAct = () => aead.Decrypt(nonce, ciphertext, tag, plaintext, badAad);
    ...
    public void RandomNonceTest()
    {
        ...
        ciphertexts.Count.ShouldBe(samples);
    }
    ...
    public void XChaCha20Poly1305TestVectors()
    {
        ...
        foreach (var test in XChaCha20Poly1305TestVector.TestVectors)
        {
            ...
            var result = new byte[test.CipherText.Length];
            aead.Decrypt(test.Nonce, test.CipherText, test.Tag, result, test.Aad);
