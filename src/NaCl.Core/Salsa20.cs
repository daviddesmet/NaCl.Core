namespace NaCl.Core
{
    ...
    public class Salsa20 : Salsa20Base
    {
        ...
        protected override void SetInitialState(Span<uint> state, ReadOnlySpan<byte> nonce, int counter)
        {
            ...
            // Words 6-7 is a 64-bit nonce, which must not be repeated for the same key. Ref: http://cr.yp.to/snuffle/xsalsa-20081128.pdf under 2. Specification - Review of Salsa20
            state[6] = ArrayUtils.LoadUInt32LittleEndian(nonce, 0);
            state[7] = ArrayUtils.LoadUInt32LittleEndian(nonce, 4);

            // Words 8-9 is a 64-bit block counter, the position of the 512-bit output block. Ref: http://cr.yp.to/snuffle/xsalsa-20081128.pdf under 2. Specification - Review of Salsa20
